...
from course.views.api.base import (
    BaseCreateAPIView,
    BaseUpdateAPIView,
    BaseDeleteAPIView,
)
    ...
    if data["questions_set"]:
        ...
        if data["questions"]:
            ...
            for question in questions:
                correct_answer = (
                    question["correct_answer"] if question["correct_answer"] else ""
                )
                question_number = (
...
def blog_detail(request, id):
    ...
    if blog.questions_set:  # Kiểm tra nếu questions_set không phải là None
        ...
        for (
            question
        ) in question_set.question_question_set.all():  # Truy cập danh sách các câu hỏi
            # Phân loại câu hỏi vào phần TOEIC
            analyst = analyze_toeic_question(
    ...
    blog_data["questions_with_parts"] = (
        question_data  # Thêm thông tin câu hỏi với phần vào response
    )
    ...
    if blog.questions_set:  # Kiểm tra nếu questions_set không phải là None
        ...
        for (
            ...
            question_data.append(
                {
                    "question": question_serialized,  # Thêm thông tin câu hỏi đã được serialize
                    "analyst": "",  # Phần của câu hỏi trong TOEIC
                }
            )
            # Nếu cần, bạn có thể lưu thông tin part vào CSDL
            # question.save()  # Nếu bạn muốn lưu vào CSDL
...
def edit_blog(request, id):
    ...
    if questions_set is None:
        return Response(
            {"message": "Question set not found"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    questions_detele_obj = (
        Question.objects.filter(question_set=questions_set)
        .exclude(id__in=[question["id"] for question in questions_data])
        .delete()
    )
